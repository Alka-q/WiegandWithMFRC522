  ...
  while (1)
  {
   ...
   //EEPROM Okuma
      for(int eCnt=0x4000; eCnt<0x4015; eCnt++){
        ...
        UART1_Settings(InBufM2);   //Başlangıç için 9600bps
        Delay_ms(50);
        ComMifare_Uart1_ISR();
        if(Ucnt == 1)
          goto label;
        ...
        UART1_Settings(InBufM2);   //Başlangıç için 19200bps
        Delay_ms(50);
        ComMifare_Uart1_ISR();
        if(Ucnt == 1)
          goto label;
        ...
        UART1_Settings(InBufM2);   //Başlangıç için 38400bps
        Delay_ms(50);
        ComMifare_Uart1_ISR();
        if(Ucnt == 1)
          goto label;
        ...
        UART1_Settings(InBufM2);   //Başlangıç için 57600bps
        Delay_ms(50);
        ComMifare_Uart1_ISR();
        if(Ucnt == 1)
          goto label;
        ...
        UART1_Settings(InBufM2);   //Başlangıç için 115200bps
        Delay_ms(50);
        ComMifare_Uart1_ISR();
        if(Ucnt == 1) 
          goto label;
...
/****Error mesajları tek fonsiyon olarak üretilebilir.(Counter vs. şeklinde)****/
/*******************************************************************************/
void AuthenticateError(){
...
//if(InBufM[0] == 0xAA && InBufM[1] == 0x00 && InBufM[2] == 0x02 && InBufM[3] == 0x81) // Arayüz uygulamsı yapılır ise aktif edilmesi gerekir.
...
void Delay_ms (uint16_t time){  
    //time = 1000 olursa 1 sn lik bir fonsiyon oluşturulmuş olur.
  for(int cnt =0; cnt < 16; cnt++){
    while(time--);
  }
}
...
/*******************************************************************************
      ...
      // Bursası için SDK yazılması gerek.
      // ilk iki byte'ı AA, 00, olmalı.
      // SendBuf[5] = 0x4D mesaj kodu.
      // " SendUart1Setting(); " Fonksiyonu yazılacak.
...
void UART1_Settings(unsigned char *BaudR)
{
  ...
  {
    /**Mesaj kodu gönderilebilir. For Ex. SendBuf[5] = 0x4D(Lütfen 0,1,2,3,4 sayılarından birini giriniz.)**/
    /*
    //UART1 Error message.
